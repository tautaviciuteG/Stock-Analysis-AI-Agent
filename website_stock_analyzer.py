...
ticker_input = st.sidebar.text_input("Įveskite akcijos tickerį:", "GOOGL").strip().upper()

st.sidebar.subheader("✍️ Rankinis IBD įvedimas")
eps_rating_input = st.sidebar.number_input("EPS rating (1-99 balai):", min_value=0, max_value=99, value=0, step=1)
smr_rating_input = st.sidebar.selectbox("SMR Rating (A-E balai):", ["-", "A", "B", "C", "D", "E"])
...
def style_main_metrics(df, price_val):
    styles = pd.DataFrame('background-color: #ffffff; color: #0f172a;', index=df.index, columns=df.columns)
    for idx, row in df.iterrows():
        ...
        if num is not None:
            if metric == "Forward P/E" and num < 18:
                ...
                is_good = True
            elif metric in ["3 Year EPS Growth Rate", "3-Year Sales Growth Rate"] and num > 25:
                is_good = True
            elif metric == "1y Target Est" and price_val is not None and num > price_val:
                is_good = True

        if is_good:
            styles.loc[idx, "Reikšmė"] = "color: #15803d; font-weight: bold; background-color: #f0fdf4;"
...
def style_price_change(df):
    ...
    for idx, row in df.iterrows():
        ...
        if num is not None:
            if num > 0:
                styles.loc[idx, "Pokytis %"] = "color: #15803d; font-weight: bold; background-color: #f0fdf4;"
            elif num < 0:
                styles.loc[idx, "Pokytis %"] = "color: #b91c1c; font-weight: bold; background-color: #fef2f2;"

    return styles

def style_insider_df(df):
    return pd.DataFrame('background-color: #ffffff; color: #0f172a;', index=df.index, columns=df.columns)
...
if ticker_input:
    with st.spinner(f"Renkami ir skaičiuojami {ticker_input} duomenys..."):
        try:
            ...
            # 1 SEKCIJA: AKCIJOS KAINOS ISTORIJA (TIK PAVADINIMAS)
            ...
            fig_price.update_layout(
                ...
                xaxis=dict(showgrid=True, gridcolor="#334155", title="Data", color="#FFFFFF"),
                yaxis=dict(showgrid=True, gridcolor="#334155", title=f"Kaina ({cur})", color="#FFFFFF"),
            )
            st.plotly_chart(fig_price, use_container_width=True)

            st.divider()

            # 2 SEKCIJA: PAGRINDINIAI RODIKLIAI
            ...
            main_metrics_data = [
                ...
                },
                {
                    ...
                    "Reikšmė": f"{info.get('forwardPE'):.2f}" if info.get("forwardPE") else "N/A",
                    "Šaltinis / pastaba": "Yahoo Finance - forwardPE",
                },
                {
                    "Rodiklis": "EPS rating",
                    "Reikšmė": str(eps_rating_input) if eps_rating_input > 0 else "N/A",
                    "Šaltinis / pastaba": "IBD (Investors.com) - rankinis įvedimas",
                },
                {
                    "Rodiklis": "3 Year EPS Growth Rate",
                    "Reikšmė": f"{eps_growth_3y:.2f}%" if eps_growth_3y is not None else "N/A",
                    "Šaltinis / pastaba": "Apskaičiuota (CAGR) iš Yahoo Finance ataskaitų",
                },
                {
                    "Rodiklis": "SMR Rating",
                ...
                },
                {
                    ...
                    "Reikšmė": f"{sales_growth_3y:.2f}%" if sales_growth_3y is not None else "N/A",
                    "Šaltinis / pastaba": "Apskaičiuota (CAGR) iš Yahoo Finance ataskaitų",
                },
                {
                    "Rodiklis": "Cash",
                ...
                },
                {
                    ...
                    "Reikšmė": f"{info.get('bookValue'):.2f} {cur}" if info.get("bookValue") else "N/A",
                    "Šaltinis / pastaba": "Yahoo Finance - bookValue (mrq)",
                },
                {
                    "Rodiklis": "ROE",
                    "Reikšmė": f"{info.get('returnOnEquity') * 100:.2f}%" if info.get("returnOnEquity") else "N/A",
                ...
                },
                {
                    ...
                    "Reikšmė": f"{info.get('targetMeanPrice'):.2f} {cur}" if info.get("targetMeanPrice") else "N/A",
            ...
            st.dataframe(
                df_main.style.apply(lambda _: style_main_metrics(df_main, price), axis=None),
            ...
            # 4 SEKCIJA: ANALITIKŲ REKOMENDACIJA
            ...
            col_rec2.metric(
                ...
                f"{info.get('recommendationMean'):.2f}" if info.get("recommendationMean") else "N/A",
            )
            col_rec3.metric(
                "Analitikų skaičius",
                str(info.get("numberOfAnalystOpinions")) if info.get("numberOfAnalystOpinions") else "N/A",
            )

            st.divider()

            # 5 SEKCIJA: REVENUES, NET INCOME
                ...
                if inc is not None and not inc.empty:
                    ...
                    for cand in ["Total Revenue", "Operating Revenue", "Revenue"]:
                        if cand in inc.index:
                            ...
                            break
                    ni_s = None
                    for cand in ["Net Income", "Net Income Common Stockholders", "Net Income Including Noncontrolling Interests"]:
                        if cand in inc.index:
                            ni_s = inc.loc[cand]
                            break
                    ...
                    if rev_s is not None and ni_s is not None:
                        ...
                        fig_fin.add_trace(go.Bar(
                            x=years, y=rev_vals, name=f"Revenues ({cur} mlrd.)",
                            marker_color="#38bdf8",
                            text=[f"{v:.1f}B" for v in rev_vals],
                            textposition="auto"
                        ))
                        fig_fin.add_trace(go.Bar(
                            x=years, y=ni_vals, name=f"Net Income ({cur} mlrd.)",
                            marker_color="#10b981",
                        ...
                        fig_fin.update_xaxes(showgrid=True, gridcolor="#334155", color="#FFFFFF", title="Metai")
                        fig_fin.update_yaxes(showgrid=True, gridcolor="#334155", color="#FFFFFF", title=f"Suma ({cur} mlrd.)")
                        st.plotly_chart(fig_fin, use_container_width=True)
                    else:
                        st.info("Nepavyko aptikti pajamų arba grynojo pelno eilutes ataskaitose.")
                else:
            ...
            try:
                ...
                if ins is not None and not ins.empty:
                    ...
                    if date_col:
                        ...
                        ins_12m = ins[ins[date_col] >= cutoff].sort_values(date_col, ascending=False)

                        if not ins_12m.empty:
                            rows = []
                            for _, r in ins_12m.iterrows():
                                val = float(r.get("Value")) if pd.notna(r.get("Value")) else 0.0
                                ...
                                monthly_insider_vol[m_key] = monthly_insider_vol.get(m_key, 0.0) + val
                                ...
                                rows.append(
                                    {
                                        ...
                                        "Sandoris": txt,
                                        "Akcijos": f"{r.get('Shares'):,.0f}" if pd.notna(r.get("Shares")) else "N/A",
            ...
            col_i3.metric("Grynasis srautas (pirkta-parduota)", format_number(net_flow, currency=cur))
            ...
            # 7 SEKCIJA: GRAFIKAS (MĖNESINĖ INSIDER APYVARTA VS. KAINA)
